	...
	case tea.MouseButtonLeft:
		...
		if msg.Action == tea.MouseActionPress {
			...
			if now.Sub(h.lastClickTime) <= doubleClickInterval &&
				deltaX <= 2 && deltaY <= 2 {
				// 双击事件处理
				if h.handleDoubleClick(msg, a, main) {
					// 重置双击检测状态
...
func (h *EventHandler) handleDoubleClick(msg tea.MouseMsg, a *model.App, main *model.Main) bool {
	menu := main.CurMenu()
	menuViews := menu.MenuViews()
	...
	if submenu != nil {
		...
		main.EnterMenu(submenu, menuTitle)
		return true
	}
